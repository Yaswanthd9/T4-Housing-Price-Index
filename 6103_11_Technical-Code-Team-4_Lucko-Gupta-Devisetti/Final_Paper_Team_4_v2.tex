...
\author{Aru Gupta, Dylan Lucko, Yashwant Sai Devisetti \\ Professor Lo\\The George Washington University\\ \text{arugupta17@gwu.edu} \\ \text{dylanlucko@gwu.edu} \\ \text{yaswanthsaidevisetti@gmail.com} \\ \text{https://github.com/Yaswanthd9/T4-Housing-Price-Index}}
\date{29 April 2022}
\maketitle
\begin{abstract}
\smallskip
...
\node [right] at (10, 1) {$D_{h}$};
\node [above] at (7, 8) {$S_{h}$};
...
%\node [right] at (7, 3.67) {$E^\prime$};
\node [right] at (10, 1) {$D_{h}$};
\node [right] at (10, 4) {$D_{h^\prime}$};
\node [above] at (6.5, 8) {$S_{h}$};
%\node [right] at (9, 9) {$S^\prime$};
